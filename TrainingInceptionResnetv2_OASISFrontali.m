...
% IMMAGINI FRONTALI, TRAINING DELLA RETE INCEPTION RESNET V2 TRANSFER LEARNING
%-----------------------  Gestione dataset --------------------------
...
for z = 1 : numel(dir(fullfile("Dataset\","disc*"))) %Scorre i dischi
    for j = 1 :numel(dir(fullfile(filepath1,"OAS1_*"))) %Scorre le cartelle nei dischi
        ...
        R = filepath1 + filepath2 + num2str(i) + "_MR1\PROCESSED\MPRAGE\T88_111"; %Path della cartella 
...
options = trainingOptions('adam',"Plots","training-progress","ValidationData",valAug,"InitialLearnRate",0.0001,"MaxEpochs",70,"ValidationFrequency",30,"MiniBatchSize", 10);
